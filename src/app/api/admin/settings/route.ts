...
const KNOWN_KEYS: { key: SettingKey; kind: 'bool' | 'number' | 'string'; label: string }[] = [
  ...
  { key: 'nvidia_api_key', kind: 'string', label: 'NVIDIA NIM API Key (Primary)' },
  { key: 'nvidia_score_model', kind: 'string', label: 'NVIDIA NIM Model (e.g. meta/llama-3.3-70b-instruct)' },
  { key: 'groq_score_model', kind: 'string', label: 'Groq AI Scoring Model (Fallback)' },
  { key: 'whisper_model', kind: 'string', label: 'Transcription Model' },
  { key: 'groq_api_key', kind: 'string', label: 'Groq API Key (Fallback)' },
